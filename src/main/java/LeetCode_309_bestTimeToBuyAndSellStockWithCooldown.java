...
public class LeetCode_309_bestTimeToBuyAndSellStockWithCooldown {
    ...
    public int maxProfit(int[] prices) {
        ...
        Integer ans = recursionWithCache(prices, dp, 0, true);
        return ans;
    }
     ...
     * @return
     */
    public static Integer recursionWithCache(int[] prices, Integer[][] dp, Integer index, boolean canBuy) {
        // base case
        // 如果没有剩余天数，或者没有剩余交易次数
        ...
        if (canBuy) {// 说明 要么 连一次交易都没有 要么上一个交易结束了
            ...
            int buy = recursionWithCache(prices, dp, index + 1, false) - prices[index];
            // 选择2 跳过当前天
            // 递归到下一天（index + 1）
            // 将状态设置为能买入(因为还没有买入）
            int skip = recursionWithCache(prices, dp, index + 1, true);
            // 存储结果
            dp[index][canBuyFlag] = Math.max(buy, skip);
        } else {// 已经交易过了 只能选择是否卖出
            // 选择1 选择当前的天 以当前的天的价位 卖出
            // TODO 冷冻期为 1 天  =>  index + 1的基础上 再 + 1
            int sell = recursionWithCache(prices, dp, index + 2, true) + prices[index];

            // 选择2 跳过当前天
            // 递归到下一天（index + 1）
            // 将状态设置为能买入(因为有买入过,接下来只能卖出））
            int skip = recursionWithCache(prices, dp, index + 1, false);
            // 存储结果
