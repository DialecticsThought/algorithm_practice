...
public class LeetCode_887_ThrowChessPiecesProblem {
	...
	public static int superEggDrop2(int kChess, int nLevel) {
		...
		for (int i = 1; i != dp.length; i++) {
			//TODO 只有一颗棋子 剩下i层楼 尝试i次
			dp[i][1] = i;
		}
		//TODO 没有枚举优化 那就 从左->右 上->下
		for (int i = 1; i != dp.length; i++) {
			for (int j = 2; j != dp[0].length; j++) {
				int min = Integer.MAX_VALUE;
				for (int k = 1; k != i + 1; k++) {
					//TODO 因为要求最差的尝试次数 所有max
	...
	public static int superEggDrop3(int kChess, int nLevel) {
		...
		//TODO 假设 剩下100层楼 3棋子  第一颗棋子 扔到15层是最优解 那么 best[100][3]=15
		int[][] best = new int[nLevel + 1][kChess + 1];
		for (int i = 1; i != dp[0].length; i++) {
			dp[1][i] = 1;
			best[1][i] = 1;
		}
		for (int i = 2; i < nLevel + 1; i++) {//TODO 从上往下
			for (int j = kChess; j > 1; j--) {//TODO 从右往左
				int ans = Integer.MAX_VALUE;
				int bestChoose = -1;
				int down = best[i - 1][j];//TODO 当前格子的上方 下限
				int up = j == kChess ? i : best[i][j + 1];//TODO 当前格子的右侧 上限 最右侧格子不优化
				for (int first = down; first <= up; first++) {//TODO 在down和up之间做枚举
					//TODO 因为要求最差的尝试次数 所有max
