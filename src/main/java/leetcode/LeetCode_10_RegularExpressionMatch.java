 ...
 */
public class LeetCode_10_RegularExpressionMatch {
	...
	// 不关心 str[0~i-1] 只关心str[si.....] 能不能被 exp[ei.....]配出来！ true false
	public static boolean process(char[] s, char[] e, int si, int ei) {
		...
		if (ei + 1 == e.length || e[ei + 1] != '*') {
			// ei + 1 不是*
			// str[si] 必须和 exp[ei] 能配上！ 不能到结尾 因为exp[ei]还有字符+ ei + 1位置的字符，不是*
		...
		while (si != s.length && (e[ei] == s[si] || e[ei] == '.')) {
			if (process(s, e, si, ei + 2)) {//利用exp[ei+1]的“*”把 exp[ei]和exp[ei+1]一起消失 让exp[ei+2] 在这个基础上 尝试
				return true;
			}
			si++;//si后移 再判断
		}
		//利用exp[ei+1]的“*”把 exp[ei]和exp[ei+1]一起消失 让exp[ei+2]
