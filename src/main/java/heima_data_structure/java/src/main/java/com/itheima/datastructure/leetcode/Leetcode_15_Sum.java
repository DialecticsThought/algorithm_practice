...
public class Leetcode_15_Sum {
     ...
     *
     * 整体流程
     * 排序输入数组：排序后的数组简化了双指针的使用和去重的处理。
     * 调用 DFS 递归：从顶层递归开始，将 n = 3，表示寻找三个数。
     * 递归拆解：在 dfs() 方法中，每次递归都会减少需要的数字个数 n，直到 n == 2，此时进入 twoSum() 方法寻找两数之和。
     * 回溯和去重：DFS 通过回溯（stack.pop()）来取消当前的选择，跳过重复元素以避免生成重复结果。
     * 递归讲解
     *                   dfs(3, 0, 5, 0)
     *                  /       |       \
     *              -4          -1       0
     *              /           /  \       \
     *      dfs(2,1,5,4) dfs(2,2,5,1) dfs(2,4,5,0)
     *        (no pair)   [-1, -1, 2]   [-1, 0, 1]
     *
     * 第一层 (n=3, k=0)：
     * 循环从数组的第一个元素 -4 开始。当前栈为 [-4]。
     * 调用递归 dfs(2, 1, 5, 4)（目标和被更新为 0 - (-4) = 4）。
     *
     * 第二层 (n=2, k=1)：
     * 因为 n == 2，达到了递归的基线条件，开始进行两数之和查找。
     * 调用 twoSum(1, 5)，目标和为 4。使用双指针法查找两个数的和为 4。
     * 迭代：-1 + 2 = 1（小于4，左指针右移）。
     * 迭代：-1 + 2 = 1（同样小于4，左指针右移）。
     * 迭代：0 + 2 = 2（仍然小于4，继续右移）。
     * 找不到和为 4 的组合，返回。
     *
     * 回到第一层 (n=3, k=1)：
     * 接着处理下一个数字 -1，栈变为 [-1]。
     * 调用 dfs(2, 2, 5, 1)（目标和更新为 0 - (-1) = 1）。
     *
     * 第二层 (n=2, k=2)：
     * 又回到了两数之和问题，目标和为 1。
     * 调用 twoSum(2, 5)：
     * 迭代：-1 + 2 = 1，匹配成功！找到组合 [-1, -1, 2]，并加入结果列表。
     * 继续移动指针，避免重复。
     *
     * 回到第一层 (n=3, k=2)：
     * 下一个数字也是 -1，跳过以避免重复。
     ...
     * 回到第一层 (n=3, k=3)：
     * 继续处理数字 0，栈变为 [0]。
     * 调用 dfs(2, 4, 5, 0)（目标和为 0）。
     *
     * 第二层 (n=2, k=4)：
     * 调用 twoSum(4, 5)：
     * 迭代：1 + 2 = 3，大于目标和 0，因此右指针左移。
     * 没有找到和为 0 的组合
     * </pre>
     *
     * @param nums
    ...
    /**
     * @param n      固定的数字的个数 也是 当前需要找的数字个数。在初始调用时，n = 3，因为我们要找三个数。
     *               当 n 减少到 2 时，递归会调用 twoSum() 方法来寻找两个数
     * @param i      当前递归过程中搜索的起始下标，表示从数组的哪一个位置开始考虑元素
     * @param j      当前递归过程中搜索的结束下标，表示到数组的哪一个位置结束
     * @param target 当前的目标和。在最开始的调用中，目标和是 0，随着递归的进行，目标和会根据已经选择的元素更新
     * @param nums   已排序的整数数组本身
     * @param stack  用于存储当前递归路径中的选定元素。它类似一个栈，递归深入时添加元素，回溯时弹出元素
     * @param result 最终答案
     */
