...
public class ShellSort {
    public static void sort(int[] a) {
        ...
        for (int gap = a.length / 2; gap >= 1; gap = gap / 2) {
            ...
            //
            /**
             * TODO 前这一轮（gap 值）下，所有元素进行插入排序的起点开始，向后遍历整个数组
             * low 代表当前要进行插入排序的元素索引，从 gap 开始。
             * 之所以从 gap 开始，而不是 0，是因为：
             *      对于 gap = 4，索引 0, 1, 2, 3 组成了初始的“分组”
             *      索引 4, 5, 6, ... 开始是对这些“分组”进行插入排序
             *      这样可以 保证当前 low 之前的部分已经有序（类似插入排序的思想）
             */
