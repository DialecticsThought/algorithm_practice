 ...
 *        兔走的距离是龟的两倍，所以龟兔相遇的那一时刻 龟走的 = 兔走的 - 龟走的 = 绕环 n 圈
 *      而前面分析过，如果走 a + 绕环 n 圈，都能找到环入口，推得 => 因此从相遇点开始，再走 a 步，就是环入口
 *      但是 a 是多少 是 未知的，并不知道 在龟兔相遇的那一时刻，龟还需要走几步
 *      所以 让龟重新回到起点，兔子不动，充当第一次相遇之前的那个龟，
 *      兔子和龟 每一次都走一步， 只要它们同时走a步 不知道a是多少，但是一定能到达环的起点，并且同时到达
 *   eg:
 *                                              i -> j
 *                                           ↗          ↘
 *      a -> b -> c -> d -> e -> f -> g -> h              k
 ...
 *                                            ↖          ↙
 *                                                  l
 *     此时 兔子(y) 回到起点， 兔子和龟 同时 走一步，最终会相会于环的起点
 *      y                                        i -> j
 *      ↓                                     ↗          ↘
 *      a -> b -> c -> d -> e -> f -> g -> h               k <- x
 *                                            ↖          ↙
 ...
 *      那么从起点开始，走 7 + 绕环 n 圈（也就是 再走 5 的 n 倍） ，都能找到环入口
 *      第一次相遇时
 *        兔走了 7 + 绕环 n 圈（本例 2 圈） + k，k 是它们相遇距环入口位置（本例 3，不重要）
 *        龟走了 7 + 绕环 n 圈（本例 0 圈） + k，当然它绕的圈数比兔少
 *      兔走的距离是龟的两倍，所以龟兔相遇的那一时刻 龟走的 = 兔走的 - 龟走的 = 绕环 n 圈
 *
 * </pre>
 */
