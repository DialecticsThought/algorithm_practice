...
public class BTree {
     ...
     *       ↙          ↘
     *    [1,2,3]       [5,6]
     * 把5删去 此时少于最小key的数量  去左侧兄弟借一个key过来
     * 需要右旋
     * 先把父节点4旋转下来
     *           [4]
     *      ↙          ↘
     ...
     *       ↙          ↘
     *    [1,2]       [4,5,6]
     * 把1删去 此时少于最小key的数量 去右侧兄弟借一个key过来
     * 需要左旋
     * 先把父节点3旋转下来
     *            [3]
     *       ↙          ↘
     ...
     *       ↙          ↘
     *    [1,2]       [4,5]
     * 把4删去 此时少于最小key的数量   此时不能去左和右侧兄弟借一个key过来
     *          [3]
     *      ↙          ↘
     *    [1,2]       [5]
     ...
     *       ↙        ↘
     *    [1,2]       [4,5]
     * 把2删去 此时少于最小key的数量   此时不能去左和右侧兄弟借一个key过来
     * 此时  先2层树  2个节点
     *    [3]
     *     ↓
     *    [1]         [4,5]被单独出来
    ...
    private void balance(Node parent, Node x, int i) {
         ...
         *        ↙     ↓    ↓
         *     [4.5] [5.5] [6.5] [7.5]   这4个表示[5,6,7]的节点
         * 此时[5,6]的子节点 需要把最右侧节点 也就是最大的节点 移动到 被调整的节点[8,10]的最左侧孩子
         *                   [4,7,11]
         *      ↙             ↙      ↘          ↘
         *  [1,2]        [5,6]        [8,10]   [12,13]
         *             ↙   ↓  ↘         ↙
         ...
         *     ↙     ↓       ↘
         *   [1,2]  [5,7]  [9,10]
         *  删除2的话 只剩下1 但是1 没有左兄弟  只能把右兄弟的所有key和父节点的某一个key 合并到被调整的节点
         * eg2:
         *            [4,8]
         *      ↙      ↓       ↘
         *   [1,2]    [5,7]     [9,10]
         ...
         * 这里不是i-1 因为 被调整节点索引就是i 因为没有左孩子 i=0
         *           [8]
         *     ↙            ↘
         *   [1,4]        [9,10]            [5,7] 被单独出来
         * 最后就把被调整节点的所有key合并
