 ...
 * 所有叶子都在同一水平。
 * 根至少有两个子项。
 * 除 root 之外，每个节点最多可以有 m 个子节点和至少 m/2 个子节点。
 * 每个节点最多可以包含 m - 1 个key，最小可以包含 ⌈m/2⌉ - 1 个key。
 * B 树和 B+ 树的比较
...
public class BPlusTree {
     ...
     * 4.如果 k > k2，则选择 k3、k4,...Km-1 与步骤 2 和 3 相同。
     * 5.重复上述步骤，直到到达叶节点。
     * 6.如果叶节点中存在 k，则返回 true，否则返回 false。
     * eg:
     * <pre>
     *     			25
     *     		↙        	↘
     ...
     * searching the appropriate leaf, inserting the element and balancing/splitting the tree.
     * 将元素插入到 B+ 树中包括三个主要事件：搜索适当的叶子、插入元素和平衡/拆分树。
     * Insertion Operation 插入操作
     * Before inserting an element into a B+ tree, these properties must be kept in mind.
     * 在将元素插入到 B+ 树之前，必须牢记这些属性。
     * <p>
     ...
     * 每个节点最多可以包含 m - 1 个密钥，最小可以包含 ⌈m/2⌉ - 1 个密钥。
     ...
     * TODO
     * Case I 案例一
     * 如果叶子未满，请按递增顺序将key插入叶子节点。
     * Case II 案例二
     * 如果叶子已满，则按递增顺序将key插入叶子节点，并按以下方式平衡树。
     * 在第 m/2 位置断开节点。
     * 将第 m/2 键也添加到父节点。
     * 如果父节点已满，请按照步骤 2 到 3 操作。
     * <p>
     * TODO
     * eg: The elements to be inserted are 5,15, 25, 35, 45 要插入的元素是 5、15、25、35、45。
     * 1.Insert 5.  插入 5
     *      5
     * 2.Insert 15.  插入15
     *         5|15
