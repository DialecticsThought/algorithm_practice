 ...
 *     |                   |                     |                                      |
 *     |                   |                 父节点 key 会变化                    删除父节点中的 key(因为子节点数量变少了)
 ...
 *   |               |                     |                             |                                         |                                                     |
 *   |               |                     |                       当前节点所有key +                              当前节点所有key +                                          |
 *   |               |                     |                       左兄弟节点所有key +                           右兄弟节点所有key +                                         |
 *   |               |                     |              其父节点的连接当前节点和兄弟节点的某个key           其父节点的连接当前节点和兄弟节点的某个key                             |
 *   |               |                     |                       合并成一个新的节点                              合并成一个新的节点                                         |
 *   |               |             当前节点的父节点 key 下移                |                                         |                                                     |
 *   |               |                     |                  删除当前节点的父节点中的 被上一步合并的key     删除当前节点的父节点中的 被上一步合并的key                             |
 *   |               |                     |                             |                                         |                                                     |
 *   |               |              右兄弟节点 key 上移                    V —————————————————>  <—————————————————— V               ——————————————————————————————————————>
 *   |               |                     |                                                 |                                     |
 *   |               |                     |                                    判断：当前节点的父节点是根节点?                         |
 ...
 * TODO
 *  针对节点没有兄弟节点的问题:
 *      叶子节点：通常都会有兄弟节点，除非是唯一的叶子节点的情况。
 *      非叶子节点：可能没有兄弟节点，但这种情况只会出现在非叶子结点是根节点的情况。
 *      eg:
 *       原始:
 *           [20]
 *          /     \
 ...
 *             |
 *          [30, 40]
 *  为什么不存在没有兄弟的非叶子节点（非根节点）？
 *      节点分布平衡：B+ 树的高度是平衡的，且每一层的非叶子节点之间通过孩子指针相互连接（类似兄弟关系）。
 *                  除根节点外，所有非叶子节点都是在满载或接近满载的情况下组织的，它们会通过分裂和合并来保持平衡，
 *                  因而每个非叶子节点都必定有左兄弟或右兄弟。
 *      插入和删除操作：无论是插入新键还是删除现有键，B+ 树总会进行调整以保持平衡。
 *                  插入时节点可能会分裂，删除时节点可能会借用兄弟节点的键或与兄弟节点合并，避免任何节点单独存
 * TODO
 *  特殊情况:
 *   根节点的删除规则
 *    情况说明：
 ...
 *
 *
 *       5, 15, 25, 45, 55, 60, 10, 20, 40, 75, 85, 90, 95, 100, 105
 * </pre>
 *
 * @Description
 * @Author veritas
