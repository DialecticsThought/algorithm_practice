 ...
 * TODO
 *  删除的特殊情况：根节点删除
 *      如果根节点只有一个元素并且删除后根节点需要合并，则整个树的高度会减小。这是B+树删除操作中的一个特殊情况。
 *  规则：
 *      1.当根节点中只剩下一个元素时，如果该元素被删除或合并，则整个树的高度会减少。
 *      2.新的根节点将会成为原来的子节点。
 *  例子：
 *      假设B+树的根节点是 [5]，左子节点是 [1, 3]，右子节点是 [6, 7]，当删除5时：
 *          1.根节点被删除，树的高度降低。
 *          2.新的根节点是 [6]，整个B+树的高度减少一级
 *
 * TODO
 *  B+树删除规则:
 ...
 *           ↙      ↓       ↘            ↙         ↓         ↘
 *     [5|6] -> [7|8| ] -> [9 |10] -> [16|17] -> [18|19] -> [20|21| ]
 *     修改父节点 因为原始 父节点  [ 7 | 10] 表示 [-∞，7) ∪ [7，10) ∪ [10，16)
 ...
 *     3.兄弟节点没富余，当前结点，兄弟节点，父结点 合并成一个新的结点 (树的高度会降低) , 然后 重复 1
 *     因为[9]也不富余 这里执行条件3
 *     那么
 *                             [ ]
 *                              ↓
