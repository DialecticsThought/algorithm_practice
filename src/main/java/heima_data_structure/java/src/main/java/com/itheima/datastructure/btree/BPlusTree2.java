 ...
 *
 *  B+树删除规则:
 *  根据不同类型的节点分为三种情况：叶子节点删除、内部节点删除和根节点删除
 *  1. 叶子节点的删除规则
 *  情况说明：
 *      叶子节点是B+树中实际存储数据的地方，删除操作通常从叶子节点开始。
 *      当删除操作发生在叶子节点时，需要确保叶子节点的元素数仍满足B+树的平衡性要求。
 *  删除步骤：
 *      1.查找并删除元素：
 *          1.从根节点出发，根据索引值逐层查找，直到找到包含要删除元素的叶子节点。
 *          2.在叶子节点中删除该元素。
 *      2.检查叶子节点是否平衡：
 *          1.删除后，检查叶子节点的元素数是否大于或等于最小度数 t-1（其中 t 是B+树的度数）。
 *          2.如果删除后叶子节点的元素数大于或等于 t-1，则操作完成，不需要进一步调整。
 *          3.如果删除后叶子节点的元素数少于 t-1，则需要进行修复。
 *      3.修复操作：重分配或合并：
 *          1.重分配：如果叶子节点的相邻兄弟节点有多余的元素，可以从兄弟节点借一个元素，并更新父节点中的索引值，以反映节点间的变化。
 *          2.合并：如果兄弟节点也无法提供多余的元素，则将当前节点和兄弟节点合并，并删除父节点中的相关索引值。
 *                 这可能导致父节点的元素数不足，进而递归向上进行修复。
 *      4.递归修复的规则：
 *          1.如果叶子节点和兄弟节点合并后，导致父节点的元素数少于最小度数，则需要递归修复父节点，直到树的上层节点为止。
 *   2. 内部节点的删除规则
 *   情况说明：
 *      内部节点不存储实际数据，而是存储索引值，用于指引搜索路径。
 *      删除内部节点中的索引值时，通常需要从子树中找到一个替代值（左子树的最大值或右子树的最小值）来填补位置。
 *      这个过程会引发递归操作，递归到叶子节点。
 *   删除步骤：
 *      1.找到要删除的索引值：
 *          1.如果要删除的元素位于内部节点中，首先需要找到对应的子树中的替代值，通常选择左子树的最大值或右子树的最小值作为替代。
 *      2.替代值递归删除：
 *          1.将找到的替代值移到内部节点，替代被删除的索引值。
 *          2.然后在替代值所在的子树中递归删除替代值。
 *      3.检查子节点的平衡性：
 *          1.如果替代值被删除后，子树中的叶子节点元素数少于最小度数，则需要对叶子节点进行重分配或合并操作，并向上修复父节点。
 *      4.父节点的修复：
 *          1.如果父节点由于子节点的合并或重分配导致元素数不足，则继续向上递归修复，直到根节点为止。
 *      递归修复的规则：
 *          内部节点删除通常会引发递归，因为替代值的删除可能导致叶子节点或子树不平衡，从而需要递归修复子节点和父节点。
 *   3.根节点的删除规则
 *    情况说明：
 *      根节点的删除是B+树中一个特殊的情况，因为根节点的删除可能会导致树的高度减少。根节点的处理方式与普通内部节点类似，但当根节点中的元素数不足时，树的层次可能会改变。
 *    删除步骤：
 *      1.删除根节点中的索引值：
 ...
 *     删除还有富余：
 *          删除之后节点的key的个数 > m / 2 - 1 删除操作结束 例如 删除22
 *                              [ 16 |   ]
 *                          ↙                ↘
 *              [ 7 | 10]                     [ 18 | 20 ]
 *           ↙      ↓       ↘            ↙         ↓         ↘
 ...
 *     删除没有富余，但是兄弟节点还有富余：
 *          兄弟节点的key有富余(> m / 2 - 1) 向兄弟节点借一个记录(key)，同时替换父节点 例如 删除 15
 *          兄弟是同一个父亲
 ...
 *           ↙      ↓       ↘            ↙         ↓         ↘
 *     [5|6] -> [7|8| ] -> [9 |10] -> [16|17] -> [18|19] -> [20|21| ]
 *     修改父节点 因为原始 父节点  [ 7 | 10] 表示 [-∞，7) ∪ [7，10) ∪ [10，+∞)
 *     现在节点[7 | 10]的10这个key的右侧孩子 是 [9 |10] 不满足 [10，+∞) ，需要修改
 *     最终
 *                              [ 16 |   ]
 *                          ↙                ↘
 *              [ 7 | 9]                     [ 18 | 20 ]
 ...
 *     [5|6] -> [7|8| ] -> [9 |10] -> [16|17] -> [18|19] -> [20|21| ]
 *     兄弟节点也不富裕，合并兄弟叶子结点：
 *          即兄弟节点合并成一个新的叶子结点，并删除父节点中的key，将当前节点指向父节点，例如删除7
 *     删除7
 *                              [ 16 |   ]
 *                          ↙                ↘
 *              [ 7 | 9]                     [ 18 | 20 ]
 ...
 *     删除叶子结点的7之后，其父节点中的7也要修改，要保证父节点的区间任然有效
 *                              [ 16 |   ]
 *                          ↙                ↘
 *              [ 8 | 9]                     [ 18 | 20 ]
 *           ↙      ↓       ↘            ↙         ↓         ↘
 *     [5|6] -> [ |8| ] -> [9 |10] -> [16|17] -> [18|19] -> [20|21| ]
 *     此时发现 对于[ |8| ] 左侧兄弟[5|6]和右侧兄弟[9 |10] 都不够借，因为借完之后左右侧兄弟都不富裕了
 *     可以和左侧合并，也可以和右侧合并，这里选择和左侧合并
 ...
 *     3.兄弟节点没富余，当前结点和兄弟节点及父结点合并成一个新的结点。 树的高度会降低 重复 1
 *     因为[9]也不富余
 *     那么
 *                           [ ]
 *                            ↓
