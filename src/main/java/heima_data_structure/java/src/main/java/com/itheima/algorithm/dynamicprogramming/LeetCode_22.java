    ...
    public List<String> generateParenthesis(int n) {
        ...
        for (int j = 2; j < n + 1; j++) {
            ...
            for (int i = 0; i < j; i++) {
                ...
                /**
                 * k1表示层嵌套的括号 ，那么就是需要外面有一个括号，所以 "(" + k1 + ")"
                 * k2 表示平级的括号 所以 "(" + k1 + ")"的基础上 + k2
                 *
                 * 当 外层 for 循环  j = 2 时 ，内层for循环 i = 0 , 1
                 * 会有 i = 0 j = 1 , i = 1  j = 0 => (0,1) (1,0)
                 * (0,1) 就表示
                 *      外层循环是 k1 = dp[0] = ""
                 *      内层循环是 k2 = dp[2 - 1 - 0] = dp[1]  = "()"
                 *      执行 dp[2].add("(" + k1 + ")" + k2) => "(" + "" + ")" + "()" = ()()
                 * (1,0) 就表示
                 *      外层循环是 k1 = dp[1] = "()"
                 *      内层循环是 k2 = dp[2 - 1 - 1] = dp[1] = ""
                 *      执行 dp[2].add("(" + k1 + ")" + k2) => "(" + "()" + ")" + "" = (())
                 *
                 * 当 外层 for 循环  j = 3 时 ，内层for循环 i = 0 , 1 , 2
                 * 会有 i = 0 j = 2 , i = 1  j = 1 , i = 2 j = 0 => (0,2) (1,1) (2,0)
                 * (0,2) 就表示
                 *      外层循环是 k1 = dp[0] = ""
                 *      内层循环是 k2 = dp[3 - 1 - 0] = dp[2]  = "() ()"  "( ( ) )"
                 *      执行 dp[3].add("(" + k1 + ")" + k2)
                 *          => "(" + "() ()" + ")" + "()" =  “(()())" + "()"
                 *          => "(" + "( ( ) )" + ")" + "()" = "((()))" + "()"
                 * (1,1) 就表示
                 *      外层循环是 k1 = dp[1] = ""
                 *      内层循环是 k2 = dp[3 - 1 - 1] = dp[1]  = "()"
                 *      执行 dp[3].add("(" + k1 + ")" + k2)
                 *          => "(" + "()" + ")" + "()" =  “(())" + "()"
                 * (2,0) 就表示
                 *      外层循环是 k1 = dp[2] = "()()"  "(())"
                 *      内层循环是 k2 = dp[3 - 2 - 1] = dp[0]  = ""
                 *      执行 dp[3].add("(" + k1 + ")" + k2)
                 *          => "(" + "()()" + ")" + "" =  “(()())"
                 *          => "(" + "(())" + ")" + "" =  “((()))"
                 */
