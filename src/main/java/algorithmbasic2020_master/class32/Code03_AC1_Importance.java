 ...
 *                  → → → → → → → → → → → → ↑
 * </pre>
 * 演示：AC自动机是如何只扫描一遍字符串就匹配出字符串所包含的全部敏感词的
 * 我们这里还是用这个字符串来演示
 * s h e r h s a y
 * 首先呢我们拿到字符串里面的第一个字母S
 * 从我们的这个根节点出发开始查找
 ...
 * 因为S不是结束节点
 * 所以我们还要继续往下查Trie树
 * 我们取到s h e r h s a y第2个字母H
 * 我们发现S下面确实是有个H节点
 * 所以我们将这个暂且指针从S节点移动到这个H这里来
 * 因为H不是结束节点
 * 所以我们还要继续往下查Trie树
 * 我们取到s h e r h s a y第3个字母E
 * <p>
 * H下面它确实存在了一个E
 * 但是我们发现E这里它标记了结束
 * 也就是说走到这个位置的时候
 ...
 * 这个节点存储的这个单词长度信息
 * 它是有两个,也就意味着我们要回溯两次
 * 一次是长度3,回溯3个长度,我们第1个敏感词匹配到的叫做SHE
 * 另一次长度2,回溯2个长度,我们第2个敏感词匹配到的叫做HE
 * <p>
 * <p>
 * 我们就再往下
 * 我们取到s h e r h s a y第4个字母R
 * 这个当前这个指针所在的节点E没有R节点
 * 所以就要通过E的FAIL指针,将指针移动到他的这个FAIL指针所指向的节点E
 * <p>
 * <p>
 * 再看当前这个节点下面是否有R节点
 * 发现确实是有个R节点
 ...
 * 这个时候就来到了R节点
 * 这个节点存储的这个单词长度信息
 * 然后意味着我们这个位置检测到了一个敏感词
 * 长度3,回溯3个长度,我们第1个敏感词匹配到的叫做HER
 * <p>
 * 我们取到s h e r h s a y第5个字母H
 * 这个当前这个指针所在的节点R没有H节点
 * 所以就要通过R的FAIL指针,将指针移动到他的这个FAIL指针所指向的节点ROOT
 * <p>
 * 在看root下面是否有H节点
 * 确实有一个H节点
 * 所以我们匹配到了H,指针移动到H
 * <p>
 * 我们取到s h e r h s a y第6个字母S
 * 然后再看下一个字母S
 * 因为我们H后面并没有S这个子节点
 * 所以就要通过H的FAIL指针,将指针移动到他的这个FAIL指针所指向的节点ROOT
 * <p>
 * 在看root下面是否有S节点
 * 确实有一个S节点
 * 所以我们匹配到了S,指针移动到S
 * <p>
 * <p>
 * 我们取到s h e r h s a y第7个字母A
 * 然后再看下一个字母A
 * 因为S有A这个子节点
 * 所以我们匹配到了A,指针移动到A
 * <p>
 * 我们取到s h e r h s a y第8个字母Y
 * 然后再看下一个字母Y
 * 因为A有Y这个子节点
 * 所以我们匹配到了Y,指针移动到Y
 * 这个节点存储的这个单词长度信息
 * 然后意味着我们这个位置检测到了一个敏感词
 * 长度3,回溯3个长度,我们第1个敏感词匹配到的叫做SAY
 * <p>
 * 这里所有字符串全部扫描完毕
 */
