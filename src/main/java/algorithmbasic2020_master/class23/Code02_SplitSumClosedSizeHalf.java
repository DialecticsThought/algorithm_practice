...
public class Code02_SplitSumClosedSizeHalf {
	...
	public static int right(int[] arr) {
		...
		//TODO 判断 是否是奇数还是偶数  只要判断 & 1是否等于1 就是奇数
		if ((arr.length & 1) == 0) {
	...
	//TODO arr[i....]自由选择，挑选的个数一定要是picks个，累加和<=rest, 离rest最近的返回
	public static int process2(int[] arr, int i, int picks, int rest) {
		if (i == arr.length) {//TODO 不仅要判断 是否没有数可以选择 还要判断可以挑选数的次数是不是0
			return picks == 0 ? 0 : -1;
		} else {
			//TODO 如果剩下的数 ==要挑数的次数
			if(arr.length-i==picks){

			}
			//TODO 不挑选当前的数
			int p1 = process2(arr, i + 1, picks, rest);
			//TODO  就是要使用arr[i]这个数 picks-1
			int p2 = -1;
			int next = -1;
			if (arr[i] <= rest) {
				next = process2(arr, i + 1, picks - 1, rest - arr[i]);
			}
			if (next != -1) {//TODO 如果当前选择的后续还是有效的话 当前选择才有效
				p2 = arr[i] + next;
			}
	...
	public static int dp(int[] arr) {
		...
		//TODO if (i == arr.length) return picks == 0 ? 0 : -1;
