 ...
 * 初始化：
 *
 * 创建一个森林，其中每个节点是一个单独的树。
 * 创建一个边集，将图中的所有边按权重从小到大排序。
 * 合并树：
 *
 * 依次从边集的最小权重边开始，检查边的两个端点是否在同一棵树中。
 * 如果不在同一棵树中，则合并这两个端点所在的树，并将该边加入到MST中。
 * 重复此过程直到MST中包含的边数为∣V∣−1。
 * <pre>
 * 		    8		7
 *  	 ①-----② ----- ③
 *   4/ | 	   2| \ 	|  \ 9
 ...
 *
 * 选择边 (5, 6, 2)：
 * 合并节点5和节点6，将边(5, 6)加入到MST中。
 * 当前MST：{(6, 7, 2), (2, 8, 2), (5, 6, 2)}
 * edge=2
 * | 	 ①      ②       ③
 * |             |
 * |             |
 ...
 *
 * 选择边 (0, 1, 4)：
 * 合并节点0和节点1，将边(0, 1)加入到MST中。
 * 当前MST：{(6, 7, 2), (2, 8, 2), (5, 6, 2), (0, 1, 4)}
 * edge=3
 * | 	 ①      ②       ③
 * |    /        |
 * |  /          |
 ...
 *
 * 选择边 (6, 8, 6)：
 * 合并节点6和节点8，将边(6, 8)加入到MST中。
 * 当前MST：{(6, 7, 2), (2, 8, 2), (5, 6, 2), (0, 1, 4), (6, 8, 6)}
 * edge=4
 * | 	 ①      ②       ③
 * |    /        |
 * |  /          |
 ...
 *
 * 选择边 (2, 3, 7)：
 * 合并节点2和节点3，将边(2, 3)加入到MST中。
 * 当前MST：{(6, 7, 2), (2, 8, 2), (5, 6, 2), (0, 1, 4), (6, 8, 6), (2, 3, 7)}
 * edge=7
 * | 	 ①      ② -----③
 * |    /        |
 * |  /          |
 ...
 *
 * 选择边 (0, 7, 8)：
 * 合并节点0和节点7，将边(0, 7)加入到MST中。
 * 当前MST：{(6, 7, 2), (2, 8, 2), (5, 6, 2), (0, 1, 4), (6, 8, 6), (2, 3, 7), (0, 7, 8)}
 * edge=8
 * | 	 ①     ② -----③
 * |    /       |
 * |  /         |
 ...
 *
 * 选择边 (3, 4, 9)：
 * 合并节点3和节点4，将边(3, 4)加入到MST中。
 * 当前MST：{(6, 7, 2), (2, 8, 2), (5, 6, 2), (0, 1, 4), (2, 3, 7), (0, 7, 8), (3, 4, 9)}
 * edge=10
 * | 	 ①     ② -----③
 * |    /       |        \
 * |  /         |         \
