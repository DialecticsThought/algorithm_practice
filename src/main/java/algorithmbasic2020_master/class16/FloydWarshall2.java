 ...
 * @Data 2024/8/6 17:09
 * <pre>
 * TODO floyd最大的特点 是没有指定源 也就是下面的概念中的点i , bellmanFord ,SPFA,dijkstra都指定点i了
 *   要计算从 点i -> 点j 的距离
 *   又因为点 i 和点 j 都是代表图中的每个点
 *   那么对于 点i 需要遍历一遍图的所有点
 *   那么对于 点j 需要遍历一遍图的所有点
 *   就需要 2 个for循环
 *   除此之外 还要查看是否成立: dist[i][j] > dist[i][k] + dist[k][j]
 *   k 这个点 也代表图中的每个点， 也需要一个遍历
 * 初始化
 *  创建距离矩阵dist
 *      如果有直接路径从节点i 到节点j，则 dist[i][j] = weight(i,j)。
 *      如果没有直接路径，则 dist[i][j] = ∞。
 *      对于所有节点 dist[i][i] = 0。
 *  创建前驱矩阵pred　
 *  　pred[i][j] 记录了最短路径中，从节点 i 到节点 j 的路径是经过哪个节点
 *      如果有直接路径从节点i 到节点j，则pred[i][j] = i 初始化
 *      如果没有直接路径，则 pred[i][j] = −1　初始化
 *      对于所有节点i，pred[i][i] = i 初始化
 * 迭代
 *  主循环：对于每个中间节点k（从 0 到n−1），执行以下操作：
 *      对于每一对节点(i,j)，检查是否通过节点k 可以得到更短的路径。
 *      如果 dist[i][j] > dist[i][k] + dist[k][j]，
 *          则更新：
