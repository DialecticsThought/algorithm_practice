 ...
 * 1 -> 6 这个元素左侧的最小值 0 -> 3,这个元素右侧的最小值 2 -> 4
 * 栈： 0 -> 3 , 2 -> 4
 * 4.遍历到3位置
 * 栈： 0 -> 3 , 2 -> 4  ,3 -> 7
 * 5.遍历到4位置  因为 5 < 7  需要让 3 -> 7弹出
 * 3 -> 7 这个元素左侧的最小值 2 -> 4,这个元素右侧的最小值 4 -> 5
 * 栈： 0 -> 3 , 2 -> 4 ,  4 -> 5
 * 6.遍历到5位置
 * 栈： 0 -> 3 , 2 -> 4  , 4 -> 5 , 5 -> 8
 * 7.遍历到6位置  因为 0 < 8  需要让 5 -> 8弹出
 * 5 -> 8 这个元素左侧的最小值 4 -> 5,这个元素右侧的最小值 6 -> 0
 * 因为 0 < 5 需要让 4 -> 5弹出
 * 4 -> 5 这个元素左侧的最小值 2 -> 4,这个元素右侧的最小值 6 -> 0
 * 因为 0 < 4 需要让 2 -> 4 弹出
 * 2 -> 4 这个元素左侧的最小值 0 -> 3,这个元素右侧的最小值 6 -> 0
 * 因为 0 < 3 需要让 2 -> 4 弹出
 * 0 -> 3 这个元素左侧的最小值 -1 -> -1,这个元素右侧的最小值 6 -> 0
 * 栈： 6 -> 0
 */
