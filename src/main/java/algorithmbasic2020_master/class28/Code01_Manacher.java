...
public class Code01_Manacher {
     ...
     *
     * 假设以 i’为中心的最大回文串的左边界和右边界分别记为左小和右小。
     *
     * 位置 i 以 index 为中心向左对称过去的位置为 i’，同理 位置 i’的回文半径也是计算过的
     *      那么以 i’为中心的最大回文串大小（pArr[i’]）必然只有三种情况
     *
     * 情况1:
     * 左小和右小完全在左大和右大内部，即以 i’为中心的最大回文串完全在以 index 为中心的最大回文串的内部
     *  [      a' ?        1        ? b'     |       b ?        1        ? a     ]
     *           左小      i'     右小                 左小'      i     右小'
     * 左大                                index                                右大
     *                                                                         pR-1
     *  a'是左小位置的前一个字符，b'是右小位置的后一个字符
     *  b是 b'以 index为中心的对称字符，a 是 a'以 index 为中心的对称字符。
     *  左小'是左小以 index 为中心的对称位置，右小'是右小以 index 为中心的对称位置。
     * 如果处在情况一下，那么以位置 i为中心的最大回文串可以直接确定，就是从右小'到左小'这一段
     * 原因:
     * 左小到右小这一段如果以 index 为回文中心，对应过去就是右小'到左小'这一段，
     * 那么右小'到左小'这一段就完全是左小到右小这一段的逆序。
     * 同时有左小到右小这一段又是回文串（以 i’为回文中心），
     * 所以右小'到左小'这一段一定也是回文串
     *
     * 也就是说，以位置 i 为中心的最大回文串起码是右小'到左小'这一段。
     * 另外，以位置 i’为中心的最大回文串只是右小'到左小'这一段，说明 a'!=b'。
     * 那么与 a'相等的 a 也必然不等于与 b'相等的 b，既然 a!=b，说明以位置 i 为中心的最大回文串就是右小'到左小'这一段，而不会扩得更大
     * "[......]"表示回文串
     *                   i'         index        i
     *                   ↓           ↓           ↓
     *  {    # C [ # a # b # a # ] d # d [ # a # b # a # ] C #    }
     ...
     * 情况2：
     * 左小和右小的左侧部分在左大和右大的外部，
     * a是左大位置的前一个字符，d 是右大位置的后一个字符
     * b 是左大'位置的后一个字符，c 是右大'位置的前一个字符
     * 左大'是左大以位置 i’为中心的对称位置，右大'是右大以位置 i 为中心的对称位置
     *
     * 如果处在情况二下，那么以位置 i 为中心的最大回文串可以直接确定，就是从右大'到右大这一段
     * 原因：
     * 1.首先左大到左大'这一段和右大'到右大这一段是关于 index 对称的，
     * 所以右大'到右大这一段是左大到左大'这一段的逆序
     *
     * 2. 同时左小到右小这一段是回文串（以i’位置为中心），
     * 那么左大到左大'这一段也是回文串，左大到左大'这一段的逆序也是回文串
     * 所以右大'到右大这一段一定是回文串
     * 也就是说，以位置 i 为中心的最大回文串起码是右大'到右大这一段
     *
     * 3. 另外，左小到右小这一段是回文串，说明 a==b，b 和 c 关于 index 对称说明 b==c，左大到右大这一段没有扩得更大，说明 a!=d，所以 d!=c
     * 说明以位置 i 为中心的最大回文串就是右大'到右大这一段，而不会扩得更大
     *
     *            i'            index          i
     *            ↓              ↓             ↓
     *  { a [ ?   1   ? ] b      |     c [ ?   1   ? ] d    }
     ...
     *
     * 情况三：
     * 左小和左大是同一个位置，即以 i’为中心的最大回文串压在了以 index 为中心的最大回文串的边界上
     * 左大与左小的位置重叠，右小'是右小位置以 index 为中心的对称位置，
     * 右大'是右大位置以 i 为中心的对称位置，可以很容易证明右小'和右大'位置也重叠。
     *
     *                         index
     *                          ↓
     *  {     [    i'   ]       |       [    i'   ]     }
     ...
     *     左大 左小                  右小' 右大'
     *
     * 如果处在情况3下，那么以位置 i 为中心的最大回文串起码是右大'和右大这一段，但可能会扩得更大。
     * 因为右大'和右大这一段是左小和右小这一段以 index 为中心对称过去的，所以两段互为逆序关系
     * 同时左小和右小这一段又是回文串，所以右大'和右大这一段肯定是回文串，
