...
public class Code01_SizeBalancedTreeMap {
	...
	public static class SizeBalancedTreeMap<K extends Comparable<K>, V> {
		...
		/*
		* TODO 每当某个节点执行完maintain方法后去其父节点递归执行该操作
		*  每次执行maintain都会返回新的root节点
		* */
		private SBTNode<K, V> maintain(SBTNode<K, V> cur) {
			if (cur == null) {
				return null;
			}
			//TODO 左孩子的大小 左左孩子的大小  左右孩子的大小  右孩子的大小  右左孩子的大小 右右孩子的大小
			...
			if (leftLeftSize > rightSize) {//TODO LL型的违规 对应的操作
				//一次旋转
				cur = rightRotate(cur);
				cur.r = maintain(cur.r);
				cur = maintain(cur);
			} else if (leftRightSize > rightSize) {//TODO LR
				/*
				* TODO 两次旋转 3个节点的孩子节点发生变化 3个节点需要递归
				* */
				...
				cur.r = maintain(cur.r);
				cur = maintain(cur);
			} else if (rightRightSize > leftSize) {//TODO RR
				//一次旋转
				cur = leftRotate(cur);
				cur.l = maintain(cur.l);
				cur = maintain(cur);
			} else if (rightLeftSize > leftSize) {//TODO RL
				/*
				 * TODO 两次旋转 3个节点的孩子节点发生变化 3个节点需要递归
				 * */
		...
		/*
		  TODO 现在，以cur为头的树上，新增，加(key, value)这样的记录
		   加完之后，会对cur做检查，该调整调整
		   返回，调整完之后，整棵树的新头部
		*/
		private SBTNode<K, V> add(SBTNode<K, V> cur, K key, V value) {
			if (cur == null) {
				...
				cur.size++;
				if (key.compareTo(cur.key) < 0) {
					cur.l = add(cur.l, key, value);//TODO 说明添加在左侧
				} else {
					cur.r = add(cur.r, key, value);//TODO 说明添加在右侧
				}
				return maintain(cur);
			}
		}
		...
		private SBTNode<K, V> delete(SBTNode<K, V> cur, K key) {
			cur.size--;
			if (key.compareTo(cur.key) > 0) {//TODO 说明在右侧
				cur.r = delete(cur.r, key);
			} else if (key.compareTo(cur.key) < 0) {//TODO 说明在左侧
