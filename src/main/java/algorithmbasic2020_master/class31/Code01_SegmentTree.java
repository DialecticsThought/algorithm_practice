 ...
 * </pre>
 * <pre>
 * 任务5：递归访问节点[2,2] 假设没有做任务4，从任务3开始
 * 1. 从根节点 [1, 10] 开始递归访问：
 *  当前区间：[1, 10]
 ...
 * 当前区间：[2, 2]
 * 操作：查询节点 [2, 2] 的值。
 * 懒标记：累加标记 8 被应用，节点值更新为原值（2）加上累加值 8，即 10。
 * 任务4的结果：
 * 节点 [2, 2] 的最终值为 10。
 * 这包含了原始值 2，以及任务1和任务2的累加标记共 8 的累加结果。
 * </pre>
 * <pre>
 * 任务5：递归访问节点 [2, 2]  假设已经做了任务4
 * 更新标记的下放过程：
 * 1.从根节点 [1, 10] 开始：
 *      目标区间 [2, 2] 位于 [1, 10]，继续访问左子节点 [1, 5]。
 * 2.访问左子节点 [1, 5]：
 *      [1, 5] 节点上存在更新标记，因为任务4中已经设置了这个标记。此时，更新标记会被下放到它的子节点 [1, 3] 和 [4, 5]。
 *      更新标记下放的效果是，子节点 [1, 3] 和 [4, 5] 的值都应该被设为 10。
 * 3.继续访问左子节点 [1, 3]：
 *      [1, 3] 接收了从 [1, 5] 下放的更新标记，并将值设为 10。此时，更新标记会进一步下放到它的子节点 [1, 2] 和 [3, 3]。
 * 4.继续访问左子节点 [1, 2]：
 *      [1, 2] 接收了从 [1, 3] 下放的更新标记，并将值设为 10。此时，更新标记会进一步下放到它的子节点 [1, 1] 和 [2, 2]。
 * 5.访问目标节点 [2, 2]：
 *      [2, 2] 接收了从 [1, 2] 下放的更新标记，并将值设为 10
 *
 *  因为未被递归访问到的节点，它们的懒标记（更新标记或累加标记）不会被下放，也不会被应用。
 *  所以，子节点 [4, 5] 及其子节点的状态在任务5中不会被改变
 * </pre>
 */
public class Code01_SegmentTree {
    ...
    public static class SegmentTree {
         ...
         * 如果要修改的区间完全覆盖当前区间
         * 直接更新这个区间，打上lazy标记
         * 如果没有完全覆盖，且当前区间有lazy标记
         * 先下传lazy标记到子区间，再清除当前区间的lazy标记
         * 如果修改区间和左儿子有交集
         * 搜索左儿子
         * 如果修改区间和右儿子有交集
         * 搜索右儿子
