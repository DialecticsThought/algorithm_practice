package algorithmbasic2020_master.class41;
...
public class Code_410_SplitArrayLargestSum {
	...
	public static int splitArray2(int[] nums, int K) {
		...
		* */
		...
		for (int i = 1; i < N; i++) {//TODO 表示最后一个画家负责所有 前几个画家不干活
			dp[i][1] = sum(sum, 0, i);
			best[i][1] = -1;
		}
		/*
		...
		*TODO
		* 每个格子都会有左侧的指导 也就是知道下限
		* 除了最后一个行格子 其他格子 会有下面的格子的指导 也就是指导上限
		*/
		for (int j = 2; j <= K; j++) {
			for (int i = N - 1; i >= 1; i--) {
				//TODO 对于每个格子dp[i][j - 1] 都有自己的下限best[i][j - 1]
				int down = best[i][j - 1];
				//TODO  如果i==N-1，也就是最后一行 则不优化上限  否则上限就是下面一个格子best[i + 1][j]
				int up = i == N - 1 ? N - 1 : best[i + 1][j];
				int ans = Integer.MAX_VALUE;
				int bestChoose = -1;
				//TODO 从下限开始尝试 尝试到上限
				for (int leftEnd = down; leftEnd <= up; leftEnd++) {
					//TODO leftEnd == -1表示左侧什么都没有 也就是除了最后一个其他所有画家什么都没负责 所以是-1
					// dp[leftEnd][j - 1]表示 0~leftEnd个画让j-1个画家负责
					int leftCost = leftEnd == -1 ? 0 : dp[leftEnd][j - 1];
					//TODO leftEnd == i 表示除了最后一个，其他所有画家把所有画都画完了 最后一个不干活 不等于的话 就是累加和
					// 假设 0~7个画 有4个画家 假设最优解是5这个画划分 0~5画 3个画家 这个从dp表获取答案 剩下的 6~7 让最后一个画家解决 就是累加和
					int rightCost = leftEnd == i ? 0 : sum(sum, leftEnd + 1, i);
					int cur = Math.max(leftCost, rightCost);
					/*
					*TODO
					...
					*/
					if (cur < ans) {//TODO 如果现在的答案 比之前的答案好 就优化
						ans = cur;
						bestChoose = leftEnd;
					}
				}
