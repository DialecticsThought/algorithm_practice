...
public class Code04_MoneyProblem {
	...
	public static long process1(int[] d, int[] p, int ability, int index) {
		if (index == d.length) {//终止位置 不花钱 返回0
			return 0;
		}
		if (ability < d[index]) {//自身能力 <index号怪兽能力 只能花钱
			return p[index] + process1(d, p, ability + d[index], index + 1);
		} else { // ability >= d[index] 可以贿赂，也可以不贿赂
			//贿赂和不贿赂之间比大小 取最小的解 也就是最优解
			return Math.min(
					p[index] + process1(d, p, ability + d[index], index + 1),//贿赂
	...
	public static long func1(int[] d, int[] p) {
		// 主函数的意义：以0的能力去0号怪兽 如果要通过后续所有的怪兽 起码多少钱
