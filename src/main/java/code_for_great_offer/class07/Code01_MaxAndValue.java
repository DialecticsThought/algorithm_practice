...
public class Code01_MaxAndValue {
     ...
     * |		答案在第30位上的状态一定是0，保留剩余的N个数，继续考察第29位，谁也不淘汰(因为谁也不行，干脆接受30位上没有1的事实)
     * |  如果有2个，
     * |		说明答案就是这两个数(直接返回答案)，因为别的数在第30位都没有1，就这两个数有。
     * |  如果有>2个，比如K个
     * |		说明答案一定只用在这K个数中去选择某两个数，因为别的数在第30位都没有1，就这K个数有。
     * |		答案在第30位上的状态一定是1，只把这K个数作为剩余的数，继续考察第29位，其他数都淘汰掉
     * |.....
     * 现在来到i位，假设剩余的数字有M个，看看这一位是1的数，有几个
     * |	  如果有0个、或者1个，说明不管怎么在M个数中选择，任何两个数&的结果在第i位上都不可能有1了
     * |	  		答案在第i位上的状态一定是0，保留剩余的M个数，继续考察第i-1位
     * |	  如果有2个，	说明答案就是这两个数(直接返回答案)，因为别的数在第i位都没有1，就这两个数有。
     * |	  如果有>2个，比如K个，说明答案一定只用在这K个数中去选择某两个数，因为别的数在第i位都没有1，就这K个数有。
     * |	  		答案在第i位上的状态一定是1，只把这K个数作为剩余的数，继续考察第i-1位，其他数都淘汰掉
     *
     * @param arr
     * @return
     */
    public static int maxAndValue2(int[] arr) {
        // arr[0...M-1]是有用的区域  arr[M....]是垃圾区
        int M = arr.length;
        int ans = 0;
