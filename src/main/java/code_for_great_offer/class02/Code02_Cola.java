...
public class Code02_Cola {
    ...
    public static int putTimes(int m, int a, int b, int c, int x) {
        ...
        /**
         * TODO
         * 1000 还剩1张 500还剩2张 => 那么说明 还剩下 总数3张 面值2000
         */
        // 之前面值的钱还剩下多少总钱数
        int preQianRest = 0;
        // 之前面值的钱还剩下多少总张数
        ...
        for (int i = 0; i < 3 && m != 0; i++) {
             ...
             * TODO
             *  买第一瓶x元的可乐 先使用掉历史的总面值 再使用当前i号面值 k张
             *  eg： 2000是历史留下的总面值 一瓶2500  i号面值是200
             *  那么 要花掉3张200元  500/200怎么实现向上取整 ==> (500+200-1) / 200
             * */
            int curQianFirstBuyZhang = (x - preQianRest + qian[i] - 1) / qian[i];
            /*
             *TODO
             *  买第一瓶x元的可乐 先使用掉历史的总面值 再使用当前i号面值  但是 i好面值也不够用
             *  eg： 2000是历史留下的总面值 一瓶2500  i号面值是200
             *  那么 要花掉3张200元  但是200只有1张
             *  那么历史总面值 2000 =》 2200 再使用当前i+1号面值
             * */

            if (zhang[i] >= curQianFirstBuyZhang) { // 如果之前的钱和当前面值的钱，能凑出第一瓶可乐
                // 凑出来了一瓶可乐也可能存在找钱的情况，
                //TODO 使用完 历史面值 再加上 i号面值的钱 之后 返回的零钱 把后面的钱的张数条
            ...
            // 用当前面值的钱，每搞定一瓶可乐，收货机会吐出多少零钱 =》 单次的找零
