...
public class Code03_FindWordInMatrix {
	...
	//TODO 每个节点 上下左右 试一遍 也就是4个分支
	public static boolean canLoop(char[][] m, int i, int j, char[] str, int k) {
		if (k == str.length) {//base case 到达终点
			return true;
		}
		//base case 越界了 或者 m[i][j]的字符 不是 str[k] 的字符
		...
		// str[k+1....]
		boolean ans = false;
		//TODO 每个节点 上下左右 试一遍 也就是4个分支 有一个能搞定  ans就是true 搞不定 ans 就是false
	...
	//TODO 不能走重复路
	// 从m[i][j]这个字符出发，能不能找到str[k...]这个后缀串
	//TODO 一旦某一个节点走过某一个分支（上下左右某一个矩阵的节点）该分支的节点 标0 防止走回头路

	//TODO 无法改动态规划 m矩阵是可变的
