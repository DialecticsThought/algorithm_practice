...
public class Code02_Mod3Max {
	...
	* 每个位置的字符可以要、也可以不要，但是！选出来的数字拼完之后的结果，在%3之后，余数一定要是规定的mod！
	* 返回在上面设定的情况下，最大的数是多少？
	* 如果存在这样的数，返回字符串的形式
	* 如果不存在这样的数，返回特殊字符串，比如"$"，代表不可能
	* 这个递归函数可以很轻易的改出动态规划
	*TODO
	* 假设i位置的数字是4 假设要了当前的数字 假设mod=3 ，4%3=1
	* 	那么来到i+1位置 的数 就要满足mod3 满足=2  因为 i位置留下的是1 那么1+2=3
	* 假设i位置的数字是5 假设要了当前的数字 假设mod=3 ，5%3=2
	* 	那么来到i+1位置 的数 就要满足mod3 满足=1  因为 i位置留下的是1 那么2+1=3
	* 假设来到i位置 当前arr[index] % mod = x
	* 那么来到i+1位置 arr[index+1] % mod = mod - x - 1
	 */
