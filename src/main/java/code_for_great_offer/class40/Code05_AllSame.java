...
public class Code05_AllSame {
	...
	/*
	*TODO  最容易理解的版本
	* arr[0]~arr[left]还没有进入阵营
	* arr[right]~arr[n]还没有进入阵营
	* left+1~right-1 已经被改成了midV
	* 在这种情况下返回 至少要变成几次 arr的值都相同
	*TODO
	...
	* midV 可能性 : arr中的最大值！
	...
	public static int process1(int[] arr, int left, int midV, int right) {
		/*
		*TODO
		* 假设中间midV是3 左侧和右侧都没有进入阵营
		* 如果arr左侧有多个3那么直接把左侧的3都进入到中间的阵营，然后查看左侧还没加入阵营的范围大小
		* */
		 ...
		 *TODO
		 * 假设中间midV是3 右侧没有进入阵营
		 * 如果arr右侧有多个3那么直接把左侧的3都进入到中间的阵营，然后查看左侧还没加入阵营的范围大小
		 * */
		...
		* */
		...
		/*
		*TODO
		* 假设midV=3 midV的left假设是5 midV的right假设是7
		* 形如：[....5 | 3....3 | 7 ...]
		* 就2种选择
		*  midV全部变成5
		*  midV全部变成7
		* 如果arr变成 [ 3....3 | 7 ...] 左侧没有数了 只能变成右侧的数字（递归）
		* 如果arr变成 [....5 | 3....3] 右侧没有数了 只能变成左侧的数字（递归）
		* */
		int p1 = Integer.MAX_VALUE;
		if (left >= 0) {
			//TODO +1是指 （当前midV变成了左侧） 这一次 + 递归
