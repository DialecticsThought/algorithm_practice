...
* 1) int querySum(L,R) : 查询arr[L...R]上的累加和  ===> 前缀和数组 解决
*TODO
* 2) int queryAim(L,R) : 查询arr[L...R]上的目标值，目标值定义如下：
*        假设arr[L...R]上的值为[a,b,c,d]，a+b+c+d = s  s是指L~R的累加和
*        目标值为 : (s-a)^2 + (s-b)^2 + (s-c)^2 + (s-d)^2
* 		 4*s^2+ a^2 +b^2+ c^2+d^2-2*(a+b+c+d)    因为a+b+c+d =x 累加和
*		 2*s^2 + a^2 +b^2+ c^2+d^2
* 因为 a b c d 4个数 所以是  2*s^2
* 如果是5个数的话 是  3*s^2 + a^2 +b^2+ c^2+d^2 +e^2
* 如果是6个数的话 是  3*s^2 + a^2 +b^2+ c^2+d^2 +e^2 +f^2
* ==> 求k个数的的目标值 就是 (k-2)*s^2 + 这k个数的平方的和
* 1.需要 累加和x 也就是利用前缀和数组
* 2.新建一个数组 每个元素的平方是什么
*TODO
* 3) int queryMax(L,R) : 查询arr[L...R]上的最大值  ===> 线段树 解决
* 要求：
