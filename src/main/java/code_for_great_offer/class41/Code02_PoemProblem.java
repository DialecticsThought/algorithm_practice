...
public class Code02_PoemProblem {
	/*
	*TODO
	* 当前来到arr[i]  判断arr[i]~arr[N-1]这个范围的子序列有多长
	* arr[i] 不参与
	* arr[i] 参与
	* 	1.从arr[i]开始 找出最短的区间满足 AABB 如果找到了 是i~j 那么从j+1开始继续递归
	* 		为什么是最短的区间 假设i=26 在26~59找到了AABB 执行f(60) 也再26~79 执行f(80)
	* 			但是 f(60) 返回子序列的长度比f(80)的大
	* 	2.从arr[i]开始 找出最短的区间满足 ABAB
	* 	3.从arr[i]开始 找出最短的区间满足 ABBA
	*	4.从arr[i]开始 找出最短的区间满足 AAAA
	* */
