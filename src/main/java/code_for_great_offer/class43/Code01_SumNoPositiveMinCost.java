...
public class Code01_SumNoPositiveMinCost {
	...
	* */
	public static int process1(int[] arr, int x, int y, int i, int sum) {
		...
		//TODO  第二选择，执行x的操作，变0 x + 后续
		int p2 = Integer.MAX_VALUE;
		int next2 = process1(arr, x, y, i + 1, sum - arr[i]);
		if (next2 != Integer.MAX_VALUE) {
			p2 = x + next2;
		}
		//TODO  第三选择，执行y的操作，变相反数 x + 后续 7 -7 -14
	...
	* 所有的数从大到小排个序
	* 假设 x >= y的话 就没有必要执行x操作
	* 只要把前几个大的数执行y操作 看看什么程度 sum累加和变成<=0了就停下
	* 假设 x < y的话
	* 从大到小 如果某个数开始 不再执行y的操作，那么这数的后面的所有数都不应该执行y的操作
	* 如果某个数开始 不再执行y的操作，那么就会执行x的操作 当某个数开始 不再执行x的操作，那么这数的后面的所有数都不应该执行x的操作
	* 用2个for循环算出 y操作和x操作的断点 x操作和无操作的断点
	* 2）
	* 一个数组 被分为了 执行y操作的部分  执行x操作部分 无操作
	* 假设 有2个相邻的元素3,3 假设x=3 y=4
	* 最优解是把3变成-3 和另一个3抵消
	* 执行y操作的部分的长度定了 这一部分统一变成负数 执行x操作部分 这一部分统一变成0
	* 看看无操作的部分和执行y操作的部分的形成的负数相加变成0抵消 ==> 2个for循环
	* 3） 最优解
	* 先求出数组的每个位置的后缀和
	* 假设现在 从头开始到y位置的累加和是-500
	* 现在不知道x操作和无操作的断点在哪里
	* 那么只要找到哪个位置的后缀和<=500的最左位置  利用二分
	* arr[9 6 3 2 2 2 1 1 1]
	* 后缀[27 18 12 9 7 5 3 2 1]
	* 假设y只有-9 那么找到<=9的最左位置  记录下来
	* 假设y只有-15 那么找到<=15的最左位置  记录下来
	* .....
	* */
	public static int minOpStep2(int[] arr, int x, int y) {
		...
		if (x >= y) { // 没有任何必要执行x操作
			...
			//执行x操作的范围就是 y位置的后一个位置 ~ left-1
			int cost = left * x;
			for (int i = 0; i < n - 1; i++) {
				// 0~i范围的数，都执行Y操作 因为当前arr[i]是后缀和 所以转成原始的arr[i]
	...
	// arr是后缀和数组， arr[l~arr.len-1]中找到值<=v的最左位置
