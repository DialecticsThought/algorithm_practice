...
public class Code01_StringKth {
	/*
	* TODO 思路：
	*  有一个str=cdb，总共长度为7，请问cdb是第几个?
	*  第一位c :
	*  以a开头，剩下长度为(0~6)的所有可能性有几个? Math.pow(26,6)
	*  +
	*  以b开头，剩下长度为(0~6)的所有可能性有几个? Math.pow(26,6)
	*  +
	*  以c开头，剩下长度为(0)的所有可能性有几个? Math.pow(26,6)
	*  第二位d :
	*  +
	*  以ca开头的情况下，剩下长度为(0~5)的所有可能性有几个? Math.pow(26,5)
	*  +
	*  以cb开头的情况下，剩下长度为(0~5)的所有可能性有几个? Math.pow(26,5)
	*  +
	*  以cc开头的情况下，剩下长度为(0~5)的所有可能性有几个? Math.pow(26,5)
	*  +
	*  以cd开头的情况下，剩下长度为(0)的所有可能性有几个? Math.pow(26,0)
	*  第三位b
	*  +
	...
	*  以cdb开头的情况下，剩下长度为(0)的所有可能性有几个?
	...
	//TODO 不管以什么开头，剩下长度为(0~len)的所有可能性有几个
