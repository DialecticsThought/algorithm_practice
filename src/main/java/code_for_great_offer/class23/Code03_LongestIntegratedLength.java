...
* 7 6 8 7 0 5 2 3 4 16
* 先把后面的子数组7 0 5 2 3 4 16复制出来 再排序
* */
public class Code03_LongestIntegratedLength {
	/*
	 * 子数组满足条件
	 * 1.元素不重复
	 * 2.子数组的最大值-最小值=元素个数-1
	 * */
	public static int maxLen(int[] arr) {
		...
		int ans = 1;//初始答案
		for (int L = 0; L < N; L++) {
			set.clear();//一旦子数组的开头 出现变化 俺么set要清空
			//初始的min和max
			int min = arr[L];
			int max = arr[L];
			set.add(arr[L]);
			// L..R 因为arr.len至少是1 那么R从L+1开始遍历
			for (int R = L + 1; R < N; R++) {
				// arr[L~R]是否可整合 1.无重复值 2.arr的max-min=arr元素个数-1
				if(set.contains(arr[R])) {//说明有重复值 这个arr不满足条件
					/*
				...
				// arr[L..R]上无重复值
				set.add(arr[R]);
				min = Math.min(min, arr[R]);//更新
				max = Math.max(max, arr[R]);//更新
