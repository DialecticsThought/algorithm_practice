...
public class Code01_LCATarjanAndTreeChainPartition {
	...
	// mq问题列表 head有哪些问题 mq[head] = {x,y,z} (head，x)之间有问题 (head，y)之间有问题 (head z)之间有问题
	...
	public static void process(int head, int[][] mt, int[][] mq, int[][] mi, UnionFind uf, int[] ans) {
		for (int next : mt[head]) { // head有哪些孩子，都遍历去吧！
			...
			//当前是递归遍历完的状态
			uf.union(head, next);//head的孩子的集合与head的集合合并
			uf.setTag(head, head);//head所处的集合的tag就是head
		}
		// 解决head的问题！
		int[] q = mq[head];
		int[] i = mi[head];
